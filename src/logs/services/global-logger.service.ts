...
@Injectable()
export class GlobalLoggerService extends ConsoleLogger implements LoggerService {
  ...
  private setupProcessHandlers() {
    process.on('uncaughtException', (error) => {
      this.error(`Uncaught Exception: ${error.message}`, error.stack, 'Process');
    });
  ...
  private broadcast(level: string, message: any, context?: string, stack?: string) {
