...
impl SqlServerDriver {
    pub async fn new(url: &str) -> AppResult<Self> {
        ...
        let tcp = TcpStream::connect(config.get_addr())
            .await
            .map_err(|e| AppError::Connection(format!("Could not reach SQL Server {}: {}", config.get_addr(), e)))?;

        tcp.set_nodelay(true)
            .map_err(|e| AppError::Connection(format!("Failed to set TCP_NODELAY for SQL Server socket: {}", e)))?;
    ...
    fn parse_url(url: &str) -> Result<Config, AppError> {
        ...
        if parsed.scheme() != "sqlserver" {
            return Err(AppError::Connection("SQL Server URL scheme must be sqlserver".into()));
        }

        let host = parsed.host_str().ok_or_else(|| AppError::Connection("SQL Server URL missing host".into()))?;
        ...
        let encrypt = parsed.query_pairs()
            .find(|(k, _)| k == "encrypt")
    ...
    fn decode_column(row: &tiberius::Row, idx: usize) -> serde_json::Value {
        ...
        if let Some(value) = row.get::<f64, _>(idx) {
            return serde_json::Number::from_f64(value).map_or(serde_json::Value::Null, serde_json::Value::Number);
        }
    ...
    async fn run_query(&self, query: &str) -> AppResult<Vec<serde_json::Value>> {
        ...
        while let Some(row) = stream.try_next().await.map_err(|e| AppError::Database(format!("SQL Server query stream failed: {}", e)))? {
...
impl DbDriver for SqlServerDriver {
    ...
    async fn fetch_schemas(&self) -> AppResult<Vec<String>> {
        let rows = self.run_query("SELECT name FROM sys.schemas ORDER BY name").await?;
    ...
    async fn fetch_columns(&self, table: &str, schema: Option<String>) -> AppResult<Vec<serde_json::Value>> {
        ...
        let query = format!(
            ...
            Self::escape_sql(table), Self::escape_sql(&schema_name)
        );
    ...
    async fn fetch_ddl(&self, name: &str, object_type: &str, schema: Option<String>) -> AppResult<String> {
        ...
        let query = format!(
            ...
            Self::escape_sql(name), Self::escape_sql(&schema_name)
        );
        ...
        if object_type.to_lowercase() == "table" {
            return Ok(format!("-- DDL for table {}.{} not IMPLEMENTED for SQL Server yet\n-- Use a specialized tool for full table DDL", schema_name, name));
        }

        Err(AppError::Internal(format!("Could not retrieve DDL for {} {}.{}", object_type, schema_name, name)))
    }
