                                ...
                                match col_status {
                                    "modified" => {
                                        if let Some(t_type) = col.get("target_type").and_then(|t| t.as_str()) {
                                            stmts.push(ScriptStatement {
                                                id: next_id(),
                                                sql: format!("ALTER TABLE \"{}\" ALTER COLUMN \"{}\" TYPE {};", obj.name, col_name, t_type),
                                                description: format!("Alter column {}.{}", obj.name, col_name),
                                            ...
                                            if nullable {
                                                stmts.push(ScriptStatement {
                                                    id: next_id(),
                                                    sql: format!("ALTER TABLE \"{}\" ALTER COLUMN \"{}\" DROP NOT NULL;", obj.name, col_name),
                                                    description: format!("Set column {}.{} nullable", obj.name, col_name),
                                                    diff_type: "alter".into(),
                                                    object_name: col_name.to_string(),
                                                    object_type: "column".into(),
                                                    selected: true,
                                                ...
                                                stmts.push(ScriptStatement {
                                                    id: next_id(),
                                                    sql: format!("ALTER TABLE \"{}\" ALTER COLUMN \"{}\" SET NOT NULL;", obj.name, col_name),
                                                    description: format!("Set column {}.{} not null", obj.name, col_name),
                                                    diff_type: "alter".into(),
                                                    object_name: col_name.to_string(),
                                                    object_type: "column".into(),
                                                    selected: true,
        ...
        for idx in &report.indexes {
            match idx.status {
                ...
                CompareStatus::Modified => {
                    ...
                    stmts.push(ScriptStatement {
                        id: next_id(),
                        sql: format!("DROP INDEX \"{}\";\nCREATE {}INDEX \"{}\" ON \"{}\" (\"{}\");", idx.name, unique_kw, idx.name, idx.table, cols.join("\", \"")),
        ...
        for fk in &report.foreign_keys {
            match fk.status {
                ...
                CompareStatus::Missing => {
                    stmts.push(ScriptStatement {
                        id: next_id(),
                        sql: format!("ALTER TABLE \"{}\" DROP CONSTRAINT \"{}\";", fk.table, fk.name),
        ...
        for view in &report.views {
            match view.status {
                CompareStatus::New | CompareStatus::Modified => {
                    stmts.push(ScriptStatement {
                        id: next_id(),
                        sql: format!("-- TODO: CREATE OR REPLACE VIEW \"{}\" (requires DDL from source)", view.name),
        ...
        for proc in &report.procedures {
            match proc.status {
                CompareStatus::New | CompareStatus::Modified => {
                    stmts.push(ScriptStatement {
                        id: next_id(),
                        sql: format!("-- TODO: CREATE OR REPLACE PROCEDURE \"{}\" (requires DDL from source)", proc.name),
        ...
        for func in &report.functions {
            match func.status {
                CompareStatus::New | CompareStatus::Modified => {
                    stmts.push(ScriptStatement {
                        id: next_id(),
                        sql: format!("-- TODO: CREATE OR REPLACE FUNCTION \"{}\" (requires DDL from source)", func.name),
