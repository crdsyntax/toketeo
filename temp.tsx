...
interface ObjectDetailProps {
  ...
  activeTab: 'columns' | 'data' | 'ddl' | 'indexes' | 'foreign-keys' | 'constraints';
  setActiveTab: (tab: 'columns' | 'data' | 'ddl' | 'indexes' | 'foreign-keys' | 'constraints') => void;
...
export function ObjectDetail({
  ...
  const [editedColData, setEditedColData] = useState<{name: string, type: string} | null>(null)
  ...
  const saveEdit = (oldName: string) => {
    ...
    const sql = `ALTER TABLE \`${selectedItem?.name}\` CHANGE COLUMN \`${oldName}\` \`${editedColData.name}\` ${editedColData.type};`
    ...
    <>
      ...
      <div className="flex-1 overflow-auto flex flex-col">
                      ...
                      >
                        <td className="py-3 font-medium text-foreground">
                          {editingColumn === col.name ? (
                            <input className="bg-muted px-1" value={editedColData?.name} onChange={e => setEditedColData(prev => prev ? {...prev, name: e.target.value} : null)} />
                          ) : col.name}
                        </td>
                        <td className="py-3 font-mono text-xs text-muted-foreground uppercase tracking-tighter">
                          {editingColumn === col.name ? (
                            <input className="bg-muted px-1" value={editedColData?.type} onChange={e => setEditedColData(prev => prev ? {...prev, type: e.target.value} : null)} />
                          ) : col.type}
                        </td>
                        ...
                        <td className="py-3 text-right space-x-2">
                          {editingColumn === col.name ? (
                            <button onClick={() => saveEdit(col.name)} className="text-primary font-bold text-xs">Save</button>
                          ) : (
                            <button onClick={() => startEdit(col)} className="text-muted-foreground hover:text-primary text-xs">Edit</button>
                          )}
                          <button onClick={() => console.log('Drop column:', col.name)} className="text-destructive hover:text-red-700 text-xs">Drop</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {activeTab === 'indexes' && selectedItem.type === 'table' && (
          <div className="p-4">
            {isLoadingIndexes ? <div>Loading...</div> : (
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="border-b border-border text-muted-foreground">
                ...
                <tbody>
                  {indexes?.map((idx, i) => (
                    <tr key={i} className="border-b border-border/50">
                      <td className="py-3">{idx.INDEX_NAME || idx.index_name || 'N/A'}</td>
                      <td className="py-3">{idx.COLUMN_NAME || idx.column_name || 'N/A'}</td>
                      <td className="py-3">{(idx.NON_UNIQUE === 0 || idx.non_unique === 0) ? 'Yes' : 'No'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === 'foreign-keys' && selectedItem.type === 'table' && (
          <div className="p-4">
            {isLoadingForeignKeys ? <div>Loading...</div> : (
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="border-b border-border text-muted-foreground">
                ...
                <tbody>
                  {foreignKeys?.map((fk, i) => (
                    <tr key={i} className="border-b border-border/50">
                      <td className="py-3">{fk.CONSTRAINT_NAME || fk.constraint_name || 'N/A'}</td>
                      <td className="py-3">{fk.COLUMN_NAME || fk.column_name || 'N/A'}</td>
                      <td className="py-3">{fk.REFERENCED_TABLE_NAME || fk.referenced_table_name || 'N/A'}</td>
                      <td className="py-3">{fk.REFERENCED_COLUMN_NAME || fk.referenced_column_name || 'N/A'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === 'constraints' && selectedItem.type === 'table' && (
          <div className="p-4">
            {isLoadingConstraints ? <div>Loading...</div> : (
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="border-b border-border text-muted-foreground">
                    <th className="pb-2 font-medium">Constraint Name</th>
                ...
                <tbody>
                  {constraints?.map((con, i) => (
                    <tr key={i} className="border-b border-border/50">
                      <td className="py-3">{con.CONSTRAINT_NAME || con.constraint_name || 'N/A'}</td>
                      <td className="py-3">{con.CONSTRAINT_TYPE || con.constraint_type || 'N/A'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                          ...
                          {queryData.rows.map((row, i) => (
                            ...
                            >
                              {queryData.columns.map((col) => (
                                <td
                                  key={col}
                                  className="p-2 border-r border-border last:border-0 truncate max-w-[200px]"
                                >
                                  {row[col] === null ? (
