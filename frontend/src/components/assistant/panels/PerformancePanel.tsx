    ...
    <div className="h-full overflow-auto p-4 space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="p-3 rounded-lg bg-muted/30 border border-border text-center">
          ...
          <p className="text-[var(--ch-text-10)] text-muted-foreground">Avg Duration</p>
        </div>
        <div className="p-3 rounded-lg bg-muted/30 border border-border text-center">
          <TrendingUp className="w-4 h-4 text-emerald-500 mx-auto mb-1" />
          <p className="text-lg font-bold text-foreground">{maxDuration}ms</p>
          <p className="text-[var(--ch-text-10)] text-muted-foreground">Max Duration</p>
        </div>
        <div className="p-3 rounded-lg bg-muted/30 border border-border text-center">
          ...
          <p className="text-[var(--ch-text-10)] text-muted-foreground">Slow Queries</p>
        </div>
        <div className="p-3 rounded-lg bg-muted/30 border border-border text-center">
          <p className="text-lg font-bold text-foreground">{totalRows.toLocaleString()}</p>
          <p className="text-[var(--ch-text-10)] text-muted-foreground">Total Rows</p>
        </div>
      </div>
            ...
            {slowQueries.slice(0, 5).map((r) => (
              <div key={r.id} className="p-2 rounded-lg bg-amber-500/5 border border-amber-500/20">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[var(--ch-text-10)] font-mono text-amber-600 font-semibold">{r.durationMs}ms</span>
                  <span className="text-[var(--ch-text-10)] text-muted-foreground">{new Date(r.executedAt).toLocaleTimeString()}</span>
                </div>
                <pre className="text-[var(--ch-text-10)] font-mono text-muted-foreground truncate">{r.sql}</pre>
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="space-y-1">
            {recent.slice(0, 10).map((r) => (
              <div key={r.id} className="flex items-center gap-2 p-1.5 rounded hover:bg-muted/30 transition-colors">
                ...
                <pre className="flex-1 text-[var(--ch-text-10)] font-mono text-muted-foreground truncate">{r.sql}</pre>
                <span className="text-[var(--ch-text-10)] font-mono text-muted-foreground shrink-0">{r.durationMs}ms</span>
              </div>
            ))}
          </div>
        </section>
      ...
      {recent.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
          ...
          <p className="text-[var(--ch-text-10)]">Execute some queries to see performance metrics</p>
        </div>
      )}
    </div>
  )
