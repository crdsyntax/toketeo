...
export function QueryHistoryPanel({
  ...
  return (
    <div className="absolute right-0 top-10 z-50 w-[520px] max-h-[70vh] flex flex-col border border-border rounded-none bg-surface shadow-2xl shadow-black/40 overflow-hidden">
      {/* Header */}
      <div className="flex items-center gap-2 px-3 py-2 border-b border-border bg-muted shrink-0">
        ...
        <span className="ml-auto text-[var(--ch-text-10)] text-muted-foreground">{history.length} entries</span>
      ...
      <div className="px-3 py-2 border-b border-border shrink-0">
        <div className="flex items-center gap-2 bg-muted px-2 py-1 rounded-none border border-border/50">
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-[var(--ch-text-10)] text-muted-foreground">{formatDate(entry.executedAt)}</span>
                        <span className="text-[var(--ch-text-10)] text-muted-foreground">{formatDuration(entry.durationMs)}</span>
                        {entry.rowCount !== undefined && entry.status === ExecutionStatus.SUCCESS && (
                          <span className="text-[var(--ch-text-10)] text-muted-foreground">{entry.rowCount} rows</span>
                        )}
                        {entry.status === ExecutionStatus.ERROR && (
                          <span className="text-[var(--ch-text-10)] text-destructive truncate max-w-[200px]">{entry.error}</span>
                        )}
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="px-3 pb-2">
                      <pre className="text-[var(--ch-text-11)] font-mono text-foreground bg-muted border border-border/50 rounded-none p-2 overflow-x-auto whitespace-pre-wrap break-all leading-relaxed max-h-40">
                        {entry.query}
                      </pre>
                    </div>
                  )}
