            ...
            {validation.is_valid ? (
              <div className="flex items-center gap-2 p-3 border border-emerald-500/20 bg-emerald-500/5">
                ...
                <span className="text-xs font-bold text-emerald-500">
                  {tables.length} tabla(s) listas para sincronizar
                </span>
              </div>
            ) : (
              <div className="p-3 border border-orange-500/20 bg-orange-500/5 space-y-1">
                {!validation.source_connection_ok && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-destructive">No se puede conectar con la base de datos origen</span>
                  </div>
                )}
                {!validation.target_connection_ok && (
                  <div className="flex items-center gap-2">
                    <XCircle className="w-3.5 h-3.5 text-destructive" />
                    <span className="text-xs text-destructive">No se puede conectar con la base de datos destino</span>
                  </div>
                )}
